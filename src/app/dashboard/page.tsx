...
export default async function DashboardPage() {
  ...
  const stats = [
    { label: "Total Sites", value: totalSites, icon: Globe, iconStyle: { backgroundColor: "#deecf9", color: "#0078d4" } },
    { label: "Total Products", value: totalProducts, icon: Package, iconStyle: { backgroundColor: "#f4f0ff", color: "#6b2fa0" } },
    { label: "Synced", value: syncedCount, icon: CheckCircle, iconStyle: { backgroundColor: "#dff6dd", color: "#107c10" } },
    { label: "Pending", value: pendingCount, icon: Clock, iconStyle: { backgroundColor: "#fff4ce", color: "#8a6914" } },
  ];
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 flex flex-col">
        <Header title="Dashboard" subtitle="Overview of your WooCommerce sites and products" />
              ...
              <table className="w-full">
                <thead>
                  <tr style={{ borderBottom: "1px solid #edebe9", backgroundColor: "#f3f2f1" }}>
                    ...
                    <th className="text-left px-3 py-1.5 text-xs font-semibold uppercase tracking-wider" style={{ color: "#605e5c" }}>Last Synced</th>
                  </tr>
                </thead>
                <tbody style={{ borderTop: "1px solid #edebe9" }}>
                  {recentSyncs.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="px-3 py-6 text-center text-xs" style={{ color: "#a19f9d" }}>
                    ...
                    recentSyncs.map((sync) => (
                      <tr key={sync.id} className="hover:bg-[#faf9f8] transition-colors" style={{ borderBottom: "1px solid #f3f2f1" }}>
                        <td className="px-3 py-1.5 text-xs font-medium" style={{ color: "#323130" }}>{sync.product.title}</td>
                        <td className="px-3 py-1.5 text-xs" style={{ color: "#605e5c" }}>{sync.site.name}</td>
                        <td className="px-3 py-1.5"><SyncStatusBadge status={sync.status} /></td>
                        <td className="px-3 py-1.5 text-xs" style={{ color: "#605e5c" }}>
